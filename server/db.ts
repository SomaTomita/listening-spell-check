...
export function recordResult(word: string, isCorrect: boolean, ts: number) {
  ...
  if (!row) {
    db.prepare(
      'INSERT INTO mistakes (word, seen, correct, streak_wrong, last_ts) VALUES (?,?,?,?,?)'
    ).run(word, 1, isCorrect ? 1 : 0, isCorrect ? 0 : 1, ts)
  } else {
    const nextSeen = row.seen + 1
    const nextCorrect = row.correct + (isCorrect ? 1 : 0)
    const nextStreakWrong = isCorrect ? 0 : row.streak_wrong + 1
    db.prepare(
      'UPDATE mistakes SET seen=?, correct=?, streak_wrong=?, last_ts=? WHERE word=?'
    ).run(nextSeen, nextCorrect, nextStreakWrong, ts, word)
  }
  ...
  if (!m) {
    ...
    db.prepare(
      'UPDATE motivation SET today_count=?, streak_days=?, week_total=?, last_updated=? WHERE id=1'
    ).run(nextToday, nextStreak, nextWeekTotal, today)
  }
}
